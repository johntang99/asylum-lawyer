...
import { HomeSectionsPanel } from '@/components/admin/panels/HomeSectionsPanel';
import { AboutSectionsPanel } from '@/components/admin/panels/AboutSectionsPanel';
...
export function ContentEditor({
  ...
  const isVideoManagedPath = (value?: string | null) =>
    typeof value === 'string' && value.startsWith('videos/');
          ...
          <div className="flex items-end gap-2 pt-4 sm:pt-0">
            ...
            <button
              ...
              onClick={() => {
                if (!activeFile) return;
                const confirmed = window.confirm(
                  `Sync current file JSON for ${siteId} (${locale})?\n\nThis sync only updates the selected file in DB.`
                );
                if (!confirmed) return;
        ...
        <div className="flex items-center gap-2">
          ...
          {activeTab === 'presets' ? (
            ...
            <div className="space-y-6 text-sm">
              ...
              {/* Sections-based pages: SEO + section panels */}
              {showSharedPanels && isHomePageFile && Array.isArray(formData?.sections) && (
                <HomeSectionsPanel
